...
class PythonScriptLinesGenerator2(CommonScriptLinesGenerator):
	...
	def generateOpenEMSScript(self, outputDir=None):
		...
		genScript += "# num_threads : Number of threads to run or 0 to set openEMS decide\n"
		...
		genScript += f"num_threads = {int(self.form.numberOfThreads.value())}\n"
		genScript += "\n"
		...
		genScript += "\n"
		genScript += "if setup_only == False:\n"
		genScript += "\tFDTD.Run(Sim_Path, verbose=3, cleanup=True, setup_only=setup_only, debug_pec=debug_pec, numThreads=num_threads)\n"

		# Write _OpenEMS.py script file to current directory.
		currDir, nameBase = self.getCurrDir()
		
		if str(self.form.outputScriptName.text()) != "":
			nameBase = str(self.form.outputScriptName.text())
